...
public class P4 {
        ...
        throws IOException // may be thrown by the scanner
    {
        ...
        try {
            ((ProgramNode)root.value).nameAnalyzer(table); // do the parse
            if(!ErrMsg.hasFatalError)System.out.println ("program finished name analysis correctly.");
            else System.out.println("Met some fatal errors in name analysis process..");
        } catch (Exception ex){
            System.err.println("Exception occured during parse: " + ex);
            System.exit(-1);
        }
        ...
        if(!ErrMsg.hasFatalError) ((ASTnode)root.value).unparse(outFile, 0);
        outFile.close();
